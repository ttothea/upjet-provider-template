...
import (
	...
	apisCluster "github.com/cps-az-provider/provider-azurerm/apis/cluster"
	apisNamespaced "github.com/cps-az-provider/provider-azurerm/apis/namespaced"
	"github.com/cps-az-provider/provider-azurerm/config"
	"github.com/cps-az-provider/provider-azurerm/internal/clients"
	controllerCluster "github.com/cps-az-provider/provider-azurerm/internal/controller/cluster"
	controllerNamespaced "github.com/cps-az-provider/provider-azurerm/internal/controller/namespaced"
	"github.com/cps-az-provider/provider-azurerm/internal/features"
	"github.com/cps-az-provider/provider-azurerm/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Azurerm").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-azurerm"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-azurerm",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Azurerm APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Azurerm APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Azurerm controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Azurerm controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Azurerm controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Azurerm controllers")
	}
